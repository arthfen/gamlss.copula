# We define the copula CDF (i.e., Pr(X <= x, Y <= y))
copula.p <- function(fname1, frm1, lnk1, fname2, frm2, lnk2, fname3, frm3, lnk3){
	...
	fun_body <- substitute({
		...
		if(COP1 == 'NO'){
			...
			p <- pbivnorm(qnorm(u1), qnorm(u2), theta)
		} else if(COP1 == 'PL'){
			# OK - tested
			# Page 164
			if(any(theta <= 0)) stop('Theta parameter out of range')

			p <- 1/2 * (theta - 1)^(-1) * (1 + (theta - 1) * (u1 + u2) - ((1 + (theta - 1) * (u1 + u2))^2 - 4 * theta * (theta - 1) * u1 * u2)^(1/2))
		} else if(COP1 == 'FR'){
			# OK - tested
			# Page 166
			if(any(theta <= 0)) stop('Theta parameter out of range')

			p <- -theta^(-1) * log((1 - exp(-theta) - (1 - exp(-theta * u1)) * (1 - exp(-theta * u2)))/(1 - exp(-theta)))
		} else if(COP1 == 'MTCJ'){
			# Untested
			# Page 169
			if(any(theta <= 0)) stop('Theta parameter out of range')
			...
			p <- 1 - ((1 - u1)^theta + (1 - u2)^theta - (1 - u1)^theta * (1 - u2)^theta)^(1/theta)
		} else if(COP1 == 'GU'){
			# OK - tested
			# Page 172
			if(any(theta <= 1)) stop('Theta parameter out of range')
			...
			x <- -log(u1)
			y <- -log(u2)
			p <- u1 * u2 * exp((x^(-theta) + y^(-theta))^(-1/theta))
		} else if(COP1 == 'HR'){
			# OK - tested
			# Page 176
			if(any(theta <= 0)) stop('Theta parameter out of range')
			...
			p <- mapply(function(t.x, t.y, t.theta, t.kappa) {
				sg <- diag(2)
				sg[1,2] <- sg[2,1] <- t.theta
